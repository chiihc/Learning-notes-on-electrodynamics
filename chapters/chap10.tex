...
这是辐射场的重要特征：
虽然辐射场本身随 $1/r$ 衰减，
但通过半径为 $r$ 的球面的总功率在 $r\rightarrow\infty$ 时保持有限。

因此只需保留式 \ref{eq:LW_E_radiation}
中的加速场：
\begin{equation}
    \boxed{
...
则
\begin{equation}
    \boldsymbol v\cdot\boldsymbol a=0,
\end{equation}
并且
\begin{equation}
    |\boldsymbol v\times\boldsymbol a|^2=v^2a^2.
\end{equation}
...
则
\begin{equation}
    \boldsymbol v\times\boldsymbol a=0.
\end{equation}
...
可以看到，在相同的加速度 $a$ 下，
纵向加速的辐射功率随 $\gamma^6$ 增长，
而横向加速仅随 $\gamma^4$ 增长。

点电荷的辐射来自加速度场：
\begin{equation}
    E_{\mathrm{rad}}\propto\frac{1}{r}.
\end{equation}
...
\begin{equation}
    ...
    \propto\frac{1}{r^2},
\end{equation}
从而可以在 $r\rightarrow\infty$ 时得到有限的总辐射功率。
...
\begin{equation}
    v\rightarrow c,
\end{equation}
Liénard 公式中的 $\gamma$ 因子使辐射功率迅速增加，
同时辐射角分布也变得越来越集中在粒子运动方向附近。
...
\end{align}
当然，上式仅为猜测。关于$\boldsymbol{F}$垂直于的分量，它得不出任何结论，它仅
告诉你平行分量的时间平均——在一个特别的时间间隔内的平均。
